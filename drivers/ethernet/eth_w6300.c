...
#define DT_DRV_COMPAT	wiznet,w6300
...
#define W6300_SPI_BLOCK_SELECT(addr)	(((addr) >> 16) & 0x1f)
#define W6300_SPI_READ_CONTROL(addr)	(W6300_SPI_BLOCK_SELECT(addr) << 3)
#define W6300_SPI_WRITE_CONTROL(addr)   \
	((W6300_SPI_BLOCK_SELECT(addr) << 3) | BIT(2))

static int w6300_spi_read(const struct device *dev, uint32_t addr,
...
static int w6300_spi_write(const struct device *dev, uint32_t addr,
			   uint8_t *data, size_t len)
{
	const struct w6300_config *cfg = dev->config;
	int ret;
...
static int w6300_readbuf(const struct device *dev, uint16_t offset, uint8_t *buf,
			 size_t len)
{
	uint32_t addr;
	size_t remain = 0;
	int ret;
	const uint32_t mem_start = W6300_Sn_RX_MEM_START;
	const uint32_t mem_size = W6300_RX_MEM_SIZE;
	...
	ret = w6300_spi_read(dev, addr, buf, len);
	if (ret || !remain) {
		return ret;
	}

	return w6300_spi_read(dev, mem_start, buf + len, remain);
}

static int w6300_writebuf(const struct device *dev, uint16_t offset, uint8_t *buf,
			  size_t len)
{
	uint32_t addr;
	size_t remain = 0;
	int ret;
	const uint32_t mem_start = W6300_Sn_TX_MEM_START;
	const uint32_t mem_size = W6300_TX_MEM_SIZE;
	...
	ret = w6300_spi_write(dev, addr, buf, len);
	if (ret || !remain) {
		return ret;
	}

	return w6300_spi_write(dev, mem_start, buf + len, remain);
}
...
static int w6300_command(const struct device *dev, uint8_t cmd)
{
	...
	while (true) {
		...
		k_busy_wait(W6300_PHY_ACCESS_DELAY);
	}
	return 0;
}
...
static void w6300_update_link_status(const struct device *dev)
{
	...
	if (w6300_spi_read(dev, W6300_PHYCFGR, &phycfgr, 1) < 0) {
		return;
	}

	if (IS_BIT_SET(phycfgr, W6300_PHYCFGR_LNK_BIT)) {
		...
		if (IS_BIT_SET(phycfgr, W6300_PHYCFGR_SPD_BIT)) {
			speed = IS_BIT_SET(phycfgr, W6300_PHYCFGR_DPX_BIT) ? LINK_FULL_100BASE
									   : LINK_HALF_100BASE;
		} else {
			speed = IS_BIT_SET(phycfgr, W6300_PHYCFGR_DPX_BIT) ? LINK_FULL_10BASE
									   : LINK_HALF_10BASE;
		}
	...
	while (true) {
		...
		if (res == 0) {
			...
			while (gpio_pin_get_dt(&(config->interrupt))) {
				...
				if (ir) {
					/* Clear interrupt */
					w6300_spi_write(dev, W6300_S0_IR, &ir, 1);
...
static int w6300_soft_reset(const struct device *dev)
{
	...
	ret = w6300_spi_write(dev, W6300_MR, &tmp, 1);
	if (ret < 0) {
		return ret;
	}
	...
	k_msleep(5);
	tmp = MR_PB;
	w6300_spi_write(dev, W6300_MR, &tmp, 1);

	/* disable interrupt */
	return w6300_spi_write(dev, W6300_SIMR, &mask, 1);
}
...
static void w6300_memory_configure(const struct device *dev)
{
	...
	w6300_spi_write(dev, W6300_Sn_RXMEM_SIZE(0), &mem, 1);
	w6300_spi_write(dev, W6300_Sn_TXMEM_SIZE(0), &mem, 1);

	mem = 0;
	for (i = 1; i < 8; i++) {
		w6300_spi_write(dev, W6300_Sn_RXMEM_SIZE(i), &mem, 1);
		w6300_spi_write(dev, W6300_Sn_TXMEM_SIZE(i), &mem, 1);
	}
}
...
static int w6300_init(const struct device *dev)
{
	...
	if (config->reset.port != NULL) {
		...
		/* See Section 5.5.1 of the W6300 datasheet
		 * Trc = 500us
		 * Tpl = 1ms
		 */
...
DEVICE_DECLARE(eth_w6300_phy_0);

static struct w6300_runtime w6300_0_runtime = {
	.tx_sem = Z_SEM_INITIALIZER(w6300_0_runtime.tx_sem,
					1,  UINT_MAX),
	.int_sem  = Z_SEM_INITIALIZER(w6300_0_runtime.int_sem,
				      0, UINT_MAX),
};
...
static const struct w6300_config w6300_0_config = {
	...
	.phy_dev = DEVICE_GET(eth_w6300_phy_0),
};

ETH_NET_DEVICE_DT_INST_DEFINE(0,
		    w6300_init, NULL,
		    &w6300_0_runtime, &w6300_0_config,
		    CONFIG_ETH_INIT_PRIORITY, &w6300_api_funcs, NET_ETH_MTU);

DEVICE_DEFINE(eth_w6300_phy_0, DEVICE_DT_NAME(DT_DRV_INST(0)) "_phy", NULL, NULL, &w6300_0_runtime,
	      &w6300_0_config, POST_KERNEL, CONFIG_ETH_INIT_PRIORITY, &w6300_phy_driver_api);
